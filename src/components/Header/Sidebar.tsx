...
import React, { createContext, useContext, useEffect, useState, Dispatch, SetStateAction, ReactElement } from "react";
...
export default function Sidebar() {
	...
	return (
		<div className={`sidebar${sidebarVisible ? " sidebar--active" : ""}`} onClick={() => setSidebarVisibility(false)}>
			<button className="sidebar__close"><BsXLg className="sidebar__close-icon" /></button>
			<nav className={`sidebar__nav${submenuActive ? " sidebar__nav--submenu" : ""}`} onClick={e => e.stopPropagation()}>
				<div className="sidebar__container">
					<button className="sidebar__search-box">
						<span className="sidebar__search-box-label">Search Jones Store</span>
						<span className="sidebar__search-box-icon"><FiSearch /></span>
					</button>
					<div className="sidebar__links">
						<ul>
							...
							<li className="sidebar__links-item sidebar__links-menu">
								<button onClick={() => { setSubmenu(ColorwaysList); setSubmenuActive(true); }}>
									<span>COLORWAYS</span>
									<IoIosArrowForward />
								</button>
							</li>
							<li className="sidebar__links-item sidebar__links-menu">
								<button onClick={() => { setSubmenu(MenCategoriesList); setSubmenuActive(true); }}>
									<span>MEN</span>
									<IoIosArrowForward />
								</button>
							</li>
							<li className="sidebar__links-item sidebar__links-menu">
								<button onClick={() => { setSubmenu(WomenCategoriesList); setSubmenuActive(true); }}>
									<span>WOMEN</span>
									<IoIosArrowForward />
								</button>
							</li>
					...
					<div className="sidebar__links-2">
						<ul>
							<li className="sidebar__links-item sidebar__back-button">
								<button onClick={() => { setSubmenuActive(false); setTimeout(() => !sidebarVisible && setSubmenu(null), 600); }}>
									<IoIosArrowBack />
									<span>BACK</span>
								</button>
							</li>
