...
export default function Form(
	{ children, method = "GET", action, beforeSubmit, afterSubmit }:
	{
		children: ReactElement[];
		method: "GET" | "POST" | "PUT" | "DELETE";
		action: string;
		beforeSubmit?: (params: { [key: string]: string }, form: HTMLFormElement) =>
			beforeSubmitReturnType | Promise<beforeSubmitReturnType>;
		afterSubmit?: (responseData: DefaultResponse, statusCode: number) =>
			void | Promise<void>;
	}
) {
	...
	const onSubmit: FormEventHandler<HTMLFormElement> = async (event) => {
		try {
			...
			const formControls: HTMLInputElement[] = Array.from(form.querySelectorAll("[name]"));
			...
			const [
				...
				shouldContinue = true
			] = await beforeSubmit?.(Object.assign({}, params), form) || [params, true];
